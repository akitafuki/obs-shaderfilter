...
static char *load_shader_from_file_internal(const char *file_name, struct dstr **visited, size_t *visited_count,
  ...
  if (!file_name || !*file_name || depth > MAX_INCLUDE_DEPTH) {
    if (depth > MAX_INCLUDE_DEPTH)
      blog(LOG_WARNING, "[obs-shaderfilter] Maximum include depth reached while including '%s'", file_name ? file_name : "");
    return NULL;
  }
  ...
  while (lines && lines[line_i] != NULL) {
    ...
    if (strncmp(trimmed, "#include", 8) == 0 && (trimmed[8] == ' ' || trimmed[8] == '\t' || trimmed[8] == '"')) {
      ...
      if (start && end && end > start + 1) {
        ...
        char *file_contents = load_shader_from_file_internal(target_path, visited, visited_count, visited_cap, depth + 1);
        if (file_contents) {
    ...
    if (strcmp(info.name, "uv_offset") == 0) {
      ...
      for (size_t annotation_index = 0; annotation_index < annotation_count; annotation_index++) {
        ...
        if (strcmp(info.name, "name") == 0 && info.type == GS_SHADER_PARAM_STRING) {
          ...
          if (info.type == GS_SHADER_PARAM_FLOAT || info.type == GS_SHADER_PARAM_VEC2 || info.type == GS_SHADER_PARAM_VEC3 ||
