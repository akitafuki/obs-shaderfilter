  ...
  for (size_t param_index = 0; param_index < param_count; param_index++) {
    ...
    case GS_SHADER_PARAM_VEC4:
      if (default_value != NULL) {
        ...
        obs_data_set_default_int(settings, param_name, (int)color_to_int(v4->x, v4->y, v4->z, v4->w));
      ...
      int_to_color((uint32_t)obs_data_get_int(settings, param_name), &param->value.vec4.x, &param->value.vec4.y,
                   &param->value.vec4.z, &param->value.vec4.w);
      break;
...
static void get_input_source(struct shader_filter_data *filter) {
  ...
  if (gs_texrender_begin(filter->input_texrender, filter->total_width, filter->total_height)) {
    ...
    if (!filter->transition)
      obs_source_process_filter_tech_end(filter->context, pass_through, filter->total_width, filter->total_height, technique);
    ...
    if (filter_to) {
      ...
      if (filter2) {
        for (size_t i = 0; i < filter->stored_param_list.num; i++) {
          ...
          for (size_t j = 0; j < filter2->stored_param_list.num; j++) {
            ...
            switch (param->type) {
            case GS_SHADER_PARAM_FLOAT:
              gs_effect_set_float(param->param, (float)param2->value.f * f + (float)param->value.f * (1.0f - f));
              break;
            case GS_SHADER_PARAM_INT:
              gs_effect_set_int(param->param, (int)((double)param2->value.i * f + (double)param->value.i * (1.0f - f)));
              break;
            case GS_SHADER_PARAM_VEC2: {
              struct vec2 v2;
      ...
      for (size_t i = 0; i < filter->stored_param_list.num; i++) {
        ...
        case GS_SHADER_PARAM_FLOAT:
          ...
          break;
        case GS_SHADER_PARAM_INT:
          gs_effect_set_int(param->param, (int)((double)param->default_value.i * f + (double)param->value.i * (1.0f - f)));
          break;
        case GS_SHADER_PARAM_VEC2: {
          struct vec2 v2;
...
void shader_filter_activate(void *data) { shader_filter_param_source_action(data, obs_source_inc_active); }

void shader_filter_deactivate(void *data) { shader_filter_param_source_action(data, obs_source_dec_active); }

void shader_filter_show(void *data) { shader_filter_param_source_action(data, obs_source_inc_showing); }

void shader_filter_hide(void *data) { shader_filter_param_source_action(data, obs_source_dec_showing); }
